...
import { UserProfile, Item, Vault, LogEntry, AppSettings, AccentColor, AppNotification, NotificationType } from './types';
...
interface DataContextType {
  ...
  refreshData: () => void;
  triggerNotification: (title: string, message: string, type: NotificationType, configKey?: keyof AppSettings['notifications']) => void;
...
const DataProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  useEffect(() => {
      ...
      const checkHealth = () => {
          ...
          if (currentSettings.notifications.backupHealth) {
             ...
             const daysSinceSync = (new Date().getTime() - lastSync.getTime()) / (1000 * 3600 * 24);
             if (daysSinceSync > 7) {
                 triggerNotification(
                     "Backup Overdue", 
                     `Your last backup was ${Math.floor(daysSinceSync)} days ago. Please sync your vault.`, 
  ...
  useEffect(() => {
    const palette = COLOR_PALETTES[settings.accentColor] || COLOR_PALETTES.violet;
  ...
  const triggerNotification = (title: string, message: string, type: NotificationType, configKey?: keyof AppSettings['notifications']) => {
      ...
      const updatedList = [newNotification, ...storageService.getNotifications()].slice(0, 50); // Keep last 50
      setNotifications(updatedList);
      storageService.saveNotifications(updatedList);

      // Trigger Browser Push if enabled and supported
      if (currentSettings.notifications.pushNotifications && 'Notification' in window && Notification.permission === 'granted') {
          new Notification(title, { body: message, icon: '/favicon.ico' });
      }
      
      // Simulate Email if enabled
      if (currentSettings.notifications.emailNotifications && (type === NotificationType.SECURITY || type === NotificationType.ALERT)) {
          console.log(`[EMAIL SENT to ${storageService.getUser().email}]: ${title} - ${message}`);
      }
  };
  ...
  const trashItems = useMemo(() => allItems.filter(i => i.deletedAt), [allItems]);
  
  const vaults = useMemo(() => allVaults.filter(v => !v.deletedAt), [allVaults]);
  const trashVaults = useMemo(() => allVaults.filter(v => v.deletedAt), [allVaults]);
  ...
  const updateItem = (updatedItem: Item) => {
    const newItems = allItems.map(i => i.id === updatedItem.id ? updatedItem : i);
    setAllItems(newItems);
    storageService.saveItems(newItems);
  ...
  const updateVault = (updatedVault: Vault) => {
    const newVaults = allVaults.map(v => v.id === updatedVault.id ? updatedVault : v);
    setAllVaults(newVaults);
    storageService.saveVaults(newVaults);
  ...
  const updateSettings = (newSettings: AppSettings) => {
      ...
      if (newSettings.notifications.pushNotifications && 'Notification' in window && Notification.permission !== 'granted') {
  ...
  return (
    <DataContext.Provider value={{ 
        items, vaults, trashItems, trashVaults, logs, settings, notifications, unreadNotificationCount,
        addItem, updateItem, deleteItem, restoreItem, permanentlyDeleteItem,
        addVault, updateVault, deleteVault, restoreVault, permanentlyDeleteVault,
        updateSettings, addLog, refreshData, triggerNotification, markNotificationsRead, clearNotifications
    }}>
      {children}
...
const AuthProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const { triggerNotification } = useData() || { triggerNotification: () => {} }; // Handle context not ready initial render
  ...
  return (
    <AuthContext.Provider value={{ user, isAuthenticated, login, loginWithBiometrics, logout, updateUserProfile, reportFailedLogin }}>
      {children}
...
const App: React.FC = () => {
  return (
    <HashRouter>
        {/* Swapped Provider Order so Auth can use Data if we refactor, but kept standard for now */}
        {/* We need DataProvider to be available to AppRoutes, and AuthProvider to wrap it */}
        <DataProvider>
          <AuthProvider>
            <AppRoutes />
          </AuthProvider>
