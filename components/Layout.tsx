...
import { Shield, Key, LayoutGrid, Settings, Lock, Search, X, PanelLeft, Trash2, Bell, Check, Trash } from 'lucide-react';
...
const SidebarLink = ({ to, icon: Icon, label, collapsed }: { to: string, icon: any, label: string, collapsed: boolean }) => {
  return (
    ...
    >
      ...
      {!collapsed && <span className="font-medium whitespace-nowrap overflow-hidden transition-opacity duration-300">{label}</span>}
...
const AppLayout: React.FC = () => {
  ...
  const { items, notifications, unreadNotificationCount, markNotificationsRead, clearNotifications } = useData();
  ...
  const hideHeader = ['/guardian', '/trash', '/settings'].some(path => location.pathname.startsWith(path));
  ...
  const getNotifIcon = (type: NotificationType) => {
      switch(type) {
          case NotificationType.SECURITY: return <Shield size={16} className="text-red-400" />;
          case NotificationType.ALERT: return <Bell size={16} className="text-orange-400" />;
          case NotificationType.SUCCESS: return <Check size={16} className="text-green-400" />;
    ...
    >
      <div className={`p-6 flex items-center ${collapsed ? 'justify-center' : 'justify-between'}`}>
        <div className="flex items-center gap-3">
            ...
            {!collapsed && (
                <h1 className="text-xl font-bold text-white tracking-tight whitespace-nowrap overflow-hidden animate-fade-in">Hushkey</h1>
            )}
        </div>
        {!collapsed && (
            <button 
      ...
      <nav className="flex-1 px-3 py-4 space-y-2 overflow-y-auto overflow-x-hidden scrollbar-thin scrollbar-thumb-gray-800">
        <SidebarLink to="/vaults" icon={LayoutGrid} label="Vaults" collapsed={collapsed} />
        <SidebarLink to="/items" icon={Key} label="All Items" collapsed={collapsed} />
        <SidebarLink to="/guardian" icon={Shield} label="Guardian" collapsed={collapsed} />
        <SidebarLink to="/trash" icon={Trash2} label="Trash" collapsed={collapsed} />
      ...
      <div className="p-4 border-t border-gray-850">
        <button
          onClick={logout}
          className={`flex items-center gap-3 px-3 py-3 w-full text-gray-400 hover:text-red-400 hover:bg-red-500/10 rounded-xl transition-colors ${
              collapsed ? 'justify-center' : ''
          }`}
          title="Lock Vault"
        >
          <Lock size={20} className="shrink-0" />
          {!collapsed && <span className="font-medium whitespace-nowrap overflow-hidden">Lock Vault</span>}
        </button>
      </div>
    </aside>
  );
        ...
        {searchQuery && (
            <div className="absolute top-full left-0 right-0 mt-2 bg-gray-900 border border-gray-800 rounded-xl shadow-2xl p-2 max-h-60 overflow-y-auto z-50">
                {items.filter(i => i.name.toLowerCase().includes(searchQuery.toLowerCase())).map(i => (
                    <NavLink to={`/items/${i.id}`} key={i.id} className="block p-2 hover:bg-gray-800 rounded-lg" onClick={() => setSearchQuery('')}>
       ...
       <div className="flex items-center gap-2">
           ...
           <div className="relative" ref={notifRef}>
               <button 
                  ...
                  className={`p-2 rounded-full transition-colors relative ${showNotifications ? 'bg-gray-800 text-white' : 'text-gray-400 hover:text-white hover:bg-gray-800'}`}
                >
                   <Bell size={24} />
                   ...
                   <div className="absolute right-0 top-full mt-2 w-80 md:w-96 bg-gray-900 border border-gray-800 rounded-xl shadow-2xl overflow-hidden z-50 animate-fade-in origin-top-right ring-1 ring-black/5">
                        <div className="p-3 border-b border-gray-800 flex items-center justify-between bg-gray-950/50">
                            ...
                            <div className="flex gap-1">
                                <button onClick={markNotificationsRead} className="p-1 hover:bg-gray-800 rounded text-gray-500 hover:text-primary-400" title="Mark all read">
                                    <Check size={16} />
                                </button>
                                <button onClick={clearNotifications} className="p-1 hover:bg-gray-800 rounded text-gray-500 hover:text-red-400" title="Clear all">
                                    <Trash size={16} />
                                </button>
                            </div>
                        </div>
                        <div className="max-h-[300px] overflow-y-auto custom-scrollbar">
                            {notifications.length === 0 ? (
                                ...
                                notifications.map(notif => (
                                    <div key={notif.id} className={`p-3 border-b border-gray-800 last:border-0 hover:bg-gray-800/50 transition-colors ${!notif.read ? 'bg-primary-900/5' : ''}`}>
                                        <div className="flex gap-3">
                                            <div className="mt-1">{getNotifIcon(notif.type)}</div>
                                            <div className="flex-1">
                                                <div className="flex justify-between items-start">
                                                    <h4 className={`text-sm font-medium ${!notif.read ? 'text-white' : 'text-gray-300'}`}>{notif.title}</h4>
                                                    {!notif.read && <div className="w-1.5 h-1.5 rounded-full bg-primary-500 mt-1.5"></div>}
                                                </div>
                                                <p className="text-xs text-gray-400 mt-1 leading-relaxed">{notif.message}</p>
                                                <span className="text-[10px] text-gray-600 mt-2 block">{new Date(notif.timestamp).toLocaleString()}</span>
                                            </div>
                                        </div>
                                    </div>
                                ))
            ...
            <button className="md:hidden p-2 text-gray-400" onClick={() => setSearchOpen(!searchOpen)}>
    ...
    <div className="min-h-screen bg-gray-950 text-gray-100 font-sans flex">
                ...
                />
                 {searchQuery && (
                    <div className="mt-2 space-y-1">
                        {items.filter(i => i.name.toLowerCase().includes(searchQuery.toLowerCase())).slice(0, 5).map(i => (
                            <NavLink to={`/items/${i.id}`} key={i.id} className="flex items-center justify-between p-3 bg-gray-900 rounded-lg" onClick={() => {setSearchOpen(false); setSearchQuery('')}}>
                                <span className="font-medium">{i.name}</span>
                                <span className="text-xs text-gray-500 px-2 py-1 bg-gray-800 rounded">{i.type}</span>
                            </NavLink>
                        ))}
                    </div>
                )}
      ...
      <div className="md:hidden fixed bottom-[60px] left-0 right-0 z-20 flex justify-center pb-1 pointer-events-none">
          ...
          >
            ...
            <div className="flex items-center gap-1.5 text-[10px] font-bold uppercase tracking-widest text-gray-400 group-hover:text-red-400">
              <Lock size={10} />
              <span>Lock</span>
            </div>
          </button>
