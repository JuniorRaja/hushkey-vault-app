...
const ConfirmationModal: React.FC<ConfirmationModalProps> = ({ 
  ...
  confirmText = 'Confirm', cancelText = 'Cancel', type = 'danger', children 
}) => {
  ...
  return (
    <div className="fixed inset-0 z-[70] flex items-center justify-center p-4 bg-black/70 backdrop-blur-sm animate-fade-in" onClick={onClose}>
        <div className="bg-gray-900 border border-gray-800 rounded-2xl w-full max-w-sm shadow-2xl overflow-hidden animate-in zoom-in-95 duration-200 relative" onClick={e => e.stopPropagation()}>
            {/* Background Pattern */}
            ...
            <div className="p-4 md:p-6 text-center relative z-10">
                <div className={`w-12 h-12 md:w-14 md:h-14 rounded-full flex items-center justify-center mx-auto mb-3 md:mb-5 ${theme.bg} ${theme.text}`}>
                    <AlertTriangle size={24} className="md:w-7 md:h-7" />
                </div>
                <h3 className="text-lg md:text-xl font-bold text-white mb-2">{title}</h3>
                <p className="text-gray-400 text-xs md:text-sm mb-6 md:mb-8 leading-relaxed">{message}</p>
