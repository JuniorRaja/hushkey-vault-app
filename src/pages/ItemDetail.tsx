...
const ItemDetail: React.FC<ItemDetailProps> = ({ isNew }) => {
        ...
        const loadData = async () => {
            if (isNew) {
                ...
                if (existing) {
                    ...
                    // Capture the current password so handleSave can detect changes
                    // without relying on the (potentially empty) store items array.
                    setOriginalPassword(existing.data?.password);
    ...
    const handleSave = async () => {
        ...
        // For new items, stamp immediately when a password is present.
        // For existing items, compare against the password that was loaded
        // when the item was fetched (originalPassword), which is reliable even
        // when the store's items array is empty (e.g., direct navigation).
        if (isNew) {
            if (finalData.password) {
                finalData.passwordLastModified = now;
            }
        } else {
            if (finalData.password !== originalPassword) {
                finalData.passwordLastModified = now;
            }
        }
    ...
    return (
        <div className="max-w-4xl mx-auto pb-20" style={isEditing && scrolled ? { paddingTop: '80px' } : {}}>
            ...
            {/* Header Actions */}
            <div
                className={`flex items-center justify-between mb-6 transition-all duration-300 ${isEditing && scrolled
