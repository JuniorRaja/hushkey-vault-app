...
export class BiometricService {
  ...
  static async register(userId: string): Promise<void> {
    try {
      ...
      if (error) throw new Error('Failed to save biometric credential: ' + error.message);
    } catch (error: any) {
      if (error.name === 'NotAllowedError') {
        ...
        throw new Error('Biometric credential already exists. Please remove it first.');
      } else {
  ...
  static async authenticate(userId: string): Promise<boolean> {
    try {
      ...
      if (error || !data) throw new Error('No biometric credential found. Please setup biometric authentication first.');
