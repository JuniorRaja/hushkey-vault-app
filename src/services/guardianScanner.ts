...
export async function runSecurityScan(
  ...
  const loginItems = items.filter(
    (i) => i.data?.password && uuidRegex.test(i.id)
  );
  ...
  const avgScore =
    loginItems.length > 0 ? Math.round(totalScore / loginItems.length) : 100;
...
export async function fetchSessionStats(userId: string) {
  const { data: devices, error } = await supabase
    .from("devices")
    .select("id, last_active, is_trusted")
    .eq("user_id", userId);

  if (error) throw error;

  const totalDevices = devices.length;
  // Consider active if seen in last 30 days
  const thirtyDaysAgo = new Date();
  thirtyDaysAgo.setDate(thirtyDaysAgo.getDate() - 30);

  const activeSessions = devices.filter(
    (d) => new Date(d.last_active) > thirtyDaysAgo
  ).length;
